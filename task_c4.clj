...
(defn notify-msg
  ...
  (if (contains? (:bill state) ware)
    (let [required (get (:bill state) ware)
          current (get (:buffer state) ware 0)
          needed (- required current)
          available @storage-atom
          take-amount (min needed available)]
      (if (pos? take-amount)
        (try
          (swap! storage-atom #(- % take-amount))
          (let [new-buffer (assoc (:buffer state) ware (+ current take-amount))
                new-state (assoc state :buffer new-buffer)]
            (if (every? (fn [[w req]] (>= (get new-buffer w 0) req)) (:bill new-state))
              (do
                (future
                  (Thread/sleep (:duration new-state))
                  (send ((new-state :target-storage) :worker) supply-msg (:amount new-state))
                  (send (:worker new-state) 
                        (fn [s] 
                          (assoc s :buffer 
                                 (reduce-kv (fn [buf w _] (assoc buf w 0)) 
                                           (:buffer s) (:bill s))))))
                (assoc new-state :buffer 
                       (reduce-kv (fn [buf w req] 
                                   (assoc buf w (- (get buf w) req)))
                                 new-buffer (:bill new-state))))
              new-state))
